...
const WebGLMacros = // when whole 'WebGLMacros' is ‘const’, all inside it are automatically ‘const’
    {
        VOG_ATTRIBUTE_VERTEX: 0,
        VDG_ATTRIBUTE_COLOR: 1,
        VDG_ATTRIBUTE_NORMAL: 2,
        VOG_ATTRIBUTE_TEXTURE: 3,
    };
...
function main() {
    // get <canvas> element
    canvas = document.getElementById("agk");
...
function resize() {
    ...
    if (canvas.width <= canvas.height)
        mat4.ortho(orthographicProjectionMatrix, -100.0, 100.0, (-100.0 *
            (canvas.height / canvas.width)), (100.0 * (canvas.height /
            canvas.width)), -100.0, 100.0);
    else
        mat4.ortho(orthographicProjectionMatrix, (-100.0 * (canvas.width /
...
function draw() {
    ...
    // animation loop
    animationHandler = requestAnimationFrame(draw, canvas);
}
